  ...
  allMISTEdat <- function(regDat, estDat) {
    ...
    if(input$eventEst == FALSE) {
      ...
      if(input$givePeakQ == TRUE) {
        ...
        regDat$site_no.y1 <- ifelse(!is.na(regDat$site_no.y2), regDat$site_no.y2, regDat$site_no.y1)
        
        regDat$event1 <- ifelse(!is.na(regDat$event2), regDat$event2, regDat$event1)
