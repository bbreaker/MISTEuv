  ...
  allMISTEdat <- function(regDat, estDat) {
    ...
    if(input$eventEst == FALSE) {
      ...
      if (input$use2 == FALSE) {
        ...
        if (input$Method == 1) {
          ...
          if(input$adjKnots == FALSE) {
            ...
            regObj <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cs"), data = regDat)
      ...
      if(input$adjKnots == FALSE) {
        ...
        regObjRise <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cs"), data = regDatRise)
        
        regObjFall <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cs"), data = regDatFall)
