                            ...
                            sidebarMenu(
                              ...
                              menuItem("Data Estimation (Event)", tabName = "tabset4",
                                       ...
                                       checkboxInput("givePeakQ",
                                                     "Use observed discharge points", 
                                                     value = FALSE),
                                       numericInput("peakToUse", 
                                                    "Observed discharge for response station", 
                                                    value = "NA"),
                                       textInput("peakDate",
...
server <- function(input, output) ({
  ...
  allMISTEdat <- function(regDat, estDat) {
    ...
    if(input$eventEst == FALSE) {
      ...
      regDat <- mutate(regDat, event = if_else(Flow.y > lag(Flow.y, 1), "rise", "fall"))
      
      regDat$event <- if_else(is.na(regDat$event), "flat", regDat$event)
      
      estDat <- mutate(estDat, event = if_else(Flow.y > lag(Flow.y, 1), "rise", "fall"))
      
      estDat$event <- if_else(is.na(estDat$event), "flat", estDat$event)
      
      if(input$givePeakQ == FALSE) {
        
        regDatRise <- dplyr::filter(regDat, event %in% c("rise", "flat"))
        
        regDatFall <- dplyr::filter(regDat, event %in% c("fall", "flat"))
        
        estDatRise <- dplyr::filter(estDat, event %in% c("rise", "flat"))
        
        estDatFall <- dplyr::filter(estDat, event %in% c("fall", "flat"))
        
      }

      if(input$givePeakQ == TRUE) {
        
        addQs <- as.numeric(input$peakToUse)
        
        addDTs <- input$peakDate
        
        addRiseFall <- input$riseFall
        
        ObsDf <- data.frame(dateTime = as.POSIXct(addDTs, format = "%Y-%m-%d %H:%M:%S", tz = "GMT"), 
                            site_no.y = "user", Flow.y = addQs, event = addRiseFall)
        
        estDat <- dplyr::left_join(x = estDat, y = ObsDf, by = "dateTime", suffix = c("1", "2"))
        
        estDat$Flow.y1 <- if_else(is.na(estDat$Flow.y1), estDat$Flow.y2, estDat$Flow.y1)
        
        estDat$site_no.y1 <- ifelse(!is.na(estDat$site_no.y2), as.character(estDat$site_no.y2), as.character(estDat$site_no.y1))
        
        estDat$event1 <- ifelse(!is.na(estDat$event2), as.character(estDat$event2), as.character(estDat$event1))
        
        estDat <- estDat[,-c(11:13)]
        
        colnames(estDat)[7:10] <- c("site_no.y", "Flow.y", "Flow_cd.y", "event")
        
        regDat <- dplyr::left_join(x = regDat, y = ObsDf, by = "dateTime", suffix = c("1", "2"))
        
        regDat$Flow.y1 <- if_else(is.na(regDat$Flow.y1), regDat$Flow.y2, regDat$Flow.y1)
        
        regDat$site_no.y1 <- ifelse(!is.na(regDat$site_no.y2), as.character(regDat$site_no.y2), as.character(regDat$site_no.y1))
        
        regDat$event1 <- ifelse(!is.na(regDat$event2), as.character(regDat$event2), as.character(regDat$event1))
        
        regDat <- regDat[,-c(11:13)]
        
        colnames(regDat)[7:10] <- c("site_no.y", "Flow.y", "Flow_cd.y", "event")
      ...
      if(input$adjKnots == FALSE) {
        ...
        regObjRise <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cs"), data = regDatRise, select = TRUE)
        
        regObjFall <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cs"), data = regDatFall, select = TRUE)
        
      }
      
      else if(input$adjKnots == TRUE) {
        
        regObjRise <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cr", k = input$knots), data = regDatRise)
        
        regObjFall <- gam(log10(Flow.y) ~ s(log10(Flow.x), bs = "cr", k = input$knots), data = regDatFall)
      ...
      datPredRise <- data.frame(dateTime = estDatRise$dateTime, estValsRise)
      
      uprRise <- gamIntervals(estValsRise, regObjRise, interval = "prediction")$upr
      
      lwrRise <- gamIntervals(estValsRise, regObjRise, interval = "prediction")$lwr
      
      datPredRise <- data.frame(dateTime = estDatRise$dateTime, Estimated = 10^(datPredRise$fit), fitUpper = 10^(uprRise), 
                                fitLower = 10^(lwrRise), standardError = datPredRise$se.fit)
      
      duansSmRise <- (sum(10^(resid(regObjRise))))/nobs(regObjRise)
      
      datPredRise$Estimated <- signif(datPredRise$Estimated*duansSmRise, 3)
      
      datPredFall <- data.frame(dateTime = estDatFall$dateTime, estValsFall)
      
      uprFall <- gamIntervals(estValsFall, regObjFall, interval = "prediction")$upr
      
      lwrFall <- gamIntervals(estValsFall, regObjFall, interval = "prediction")$lwr
      
      datPredFall <- data.frame(dateTime = estDatFall$dateTime, Estimated = 10^(datPredFall$fit), fitUpper = 10^(uprFall), 
                                fitLower = 10^(lwrFall), standardError = datPredFall$se.fit)
      
      duansSmFall <- (sum(10^(resid(regObjFall))))/nobs(regObjFall)
      
      datPredFall$Estimated <- signif(datPredFall$Estimated*duansSmFall, 3)
      
      datPred <- dplyr::bind_rows(datPredRise, datPredFall)
      
      datPred <- datPred[order(datPred$dateTime),]
      
      datPred <- datPred[!duplicated(datPred$dateTime),]
      
      datPred <- data.frame(dateTime = datPred$dateTime,
                            Estimated = signif(datPred$Estimated, 3), 
                            fitUpper = signif(datPred$fitUpper, 3),
                            fitLower = signif(datPred$fitLower, 3), 
                            standardError = signif(datPred$standardError, 3))
      
      datP <- dplyr::left_join(x = regDat, y = datPred, by = "dateTime")
