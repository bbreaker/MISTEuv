  ...
  allMISTEdat <- function(regDat, estDat) {
    ...
    if(input$eventEst == FALSE) {
      ...
      if(input$giveObsQ == TRUE) {
        ...
        addQs <- paste0(as.numeric(unlist(strsplit(input$peakToUse, ", "))), collapse = ", ")
        
        addDTs <- paste0(as.character(unlist(strsplit(input$peakDate, ", "))), collapse = ", ")
        ...
        datP <- smoothAddQ(df = datP, startSm, endSm)
