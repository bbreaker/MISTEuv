  ...
  allMISTEdat <- function(regDat, estDat) {
    ...
    if(input$eventEst == FALSE) {
      ...
      if(input$givePeakQ == TRUE) {
        ...
        estDat$site_no.y1 <- if_else(!is.na(estDat$site_no.y2), estDat$site_no.y2, estDat$site_no.y1)
        
        estDat$event1 <- if_else(!is.na(estDat$event2), estDat$event2, estDat$event1)
        
        estDat <- estDat[,-c(11:13)]
        
        colnames(estDat)[7:10] <- c("site_no.y", "Flow.y", "Flow_cd.y", "event")
        
        regDat <- dplyr::left_join(x = regDat, y = obsDf, by = "dateTime")
        
        regDat$Flow.y.x <- if_else(is.na(regDat$Flow.y.x), regDat$Flow.y.y, regDat$Flow.y.x)
        
        regDat$site_no.y.x <- if_else(is.na(regDat$site_no.y.x), regDat$site_no.y.y, regDat$site_no.y.x)
        
        regDat$event.x <- if_else(is.na(regDat$event.x), regDat$event.y, regDat$event.x)
